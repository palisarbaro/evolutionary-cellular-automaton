...
{
    layers_count = 0;
    seeds = sycl::malloc_shared<unsigned long int>(elements_count, *queue);
    auto v = rand();
    ...
    for(int layer=0;layer<layers_count;layer++){
        for(int i=0;i<layers[layer].in_size+1;i++){
            for(int j=0;j<layers[layer].out_size;j++){
                ...
                float rnd1 = ((unsigned int)(seeds[element] / 65536) % 32768)/32767.;
                rnd1*=2;
                rnd1-=1;
                seeds[element] = seeds[element] * 1103515245 + 12345;
                float rnd2 = ((unsigned int)(seeds[element] / 65536) % 32768)/32767.;
                rnd2*=2;
                rnd2-=1;
